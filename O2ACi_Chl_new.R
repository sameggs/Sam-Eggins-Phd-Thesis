...
ACi <- read.csv("/Users/eggboy/Dropbox/Science/Data/Oxygen Evolution/csvdatafiles/Ci_curves_all_species_respincluded.csv", fileEncoding="UTF-8-BOM", header = TRUE) %>%
  mutate(temp = as.factor(temp),
         ...
         O2rate_chl = as.numeric(O2rate_chl)) %>%
  filter(!is.na(O2rate_chl))

#ordering factor levels
ACi$inhibitor <- factor(ACi$inhibitor, levels=c("none", "AZ", "EZ", "DIDS"))
...
# ------------------------------- P tricornutum 20 C -----------------------------------------------------------------------------------
top = 300 #setting initial conditions for O2max
...
fit_nls_dic <- function(df) {
  bestfit <- tryCatch(
    nls(O2rate_chl ~ O2max*DICadj/(Km+DICadj), df, start=list(O2max=top,Km=100)),
...
fit_nls_co2 <- function(df) {
  bestfit <- tryCatch(
    nls(O2rate_chl ~ O2max*CO2adj/(Km+CO2adj), df, start=list(O2max=top,Km=1)),
    error = function(e) return(NULL))
...
Pt20_nls_DIC <-  nls(O2rate_chl ~ O2max*DICadj/(Km+DICadj), Pt20[which(Pt20$inhibitor == "none"),], start=list(O2max=top,Km=kdic))
Pt20_nls_DIC_AZ <-  nls(O2rate_chl ~ O2max*DICadj/(Km+DICadj), Pt20[which(Pt20$inhibitor == "AZ"),], start=list(O2max=top,Km=kdic))
Pt20_nls_DIC_EZ <-  nls(O2rate_chl ~ O2max*DICadj/(Km+DICadj), Pt20[which(Pt20$inhibitor == "EZ"),], start=list(O2max=top,Km=kdic))
Pt20_nls_CO2 <-  nls(O2rate_chl ~ O2max*CO2adj/(Km+CO2adj), Pt20[which(Pt20$inhibitor == "none"),], start=list(O2max=top,Km=kco2))
Pt20_nls_CO2_AZ <-  nls(O2rate_chl ~ O2max*CO2adj/(Km+CO2adj), Pt20[which(Pt20$inhibitor == "AZ"),], start=list(O2max=top,Km=kco2))
Pt20_nls_CO2_EZ <-  nls(O2rate_chl ~ O2max*CO2adj/(Km+CO2adj), Pt20[which(Pt20$inhibitor == "EZ"),], start=list(O2max=top,Km=kco2))
#list the models
...
CO2_chl_plot_Pt20 <- ggplot(Pt20, aes(x = CO2adj, y = O2rate_chl)) +
  theme_bw() +
  geom_ribbon(aes(ymin = lw20_CO2, ymax = up20_CO2), fill = "#D51317", colour= NA, alpha = 0.2) +
  geom_ribbon(aes(ymin = lwAZ20_CO2, ymax = upAZ20_CO2), fill = "#95C11F", colour = NA, alpha = 0.2) +
  geom_ribbon(aes(ymin = lwEZ20_CO2, ymax = upEZ20_CO2), fill = "#0094CD", colour= NA, alpha = 0.2) +
  ...
  ylab(bquote('\u03bcmol'~O[2]~hr^-1~mg~Chl~italic(a)^-1)) + 
  xlab(bquote('CO2 (\u03bcmol'~L^-1~')')) +
  scale_y_continuous(limits = c(-150, 450), breaks = seq(-150, 450, by = 75)) +
...
DIC_chl_plot_Pt20 <- ggplot(Pt20, aes(x = DICadj, y = O2rate_chl)) +
  theme_bw() +
  geom_ribbon(aes(ymin = lw20_DIC, ymax = up20_DIC), fill = "#D51317", colour= NA, alpha = 0.2) +
  geom_ribbon(aes(ymin = lwAZ20_DIC, ymax = upAZ20_DIC), fill = "#95C11F", colour = NA, alpha = 0.2) +
  geom_ribbon(aes(ymin = lwEZ20_DIC, ymax = upEZ20_DIC), fill = "#0094CD", colour= NA, alpha = 0.2) +
  geom_point(aes(fill = inhibitor, shape = inhibitor), colour = "black",size = 2) +
  scale_fill_manual(values = c("#D51317FF" ,"#95C11FFF","#0094CDFF")) +
  scale_shape_manual(values = c(23,23,23)) +
  ylab(bquote('\u03bcmol'~O[2]~hr^-1~mg~Chl~italic(a)^-1)) + 
...
Beale_Pt20 <- ggplot() +
  ...
  scale_x_continuous(limits = c(250, 400), breaks = seq(250, 400, by = 50)) +
...
fit_nls_DIC <- function(data) {
  nls_model <- nls(O2rate_chl ~ O2max * DICadj / (Km + DICadj), 
                   data = data,
...
Boot_plusdata_Pt20 <- ggplot() +
  ...
  scale_y_continuous(limits = c(220, 460), breaks = seq(220, 460, by = 40)) +
  ...
  geom_errorbarh(data = avgcoef_Pt20 %>% filter(CoefficientType == "DIC"), aes(y = VmaxO2, xmin = K - Kse, xmax = K + Kse, colour = inhibitor), colour = "black", width = 6, size = 0.4) +
  geom_errorbar(data = avgcoef_Pt20 %>% filter(CoefficientType == "DIC"), aes(x = K, ymin = VmaxO2 - O2se, ymax = VmaxO2 + O2se, colour = inhibitor), colour = "black", height = 15, size = 0.4) +
  ...
  ylab(bquote(VO[2]^'max'~'(\u03bcmol'~O[2]~hr^-1~mg~Chl~italic(a)^-1~')')) 
...
#fitting MM kinetics to whole data set
Pt4_nls_DIC <-  nls(O2rate_chl ~ O2max*DICadj/(Km+DICadj), Pt4[which(Pt4$inhibitor == "none"),], start=list(O2max=top,Km=kdic))
Pt4_nls_DIC_AZ <-  nls(O2rate_chl ~ O2max*DICadj/(Km+DICadj), Pt4[which(Pt4$inhibitor == "AZ"),], start=list(O2max=top,Km=kdic))
Pt4_nls_DIC_EZ <-  nls(O2rate_chl ~ O2max*DICadj/(Km+DICadj), Pt4[which(Pt4$inhibitor == "EZ"),], start=list(O2max=top,Km=kdic))
Pt4_nls_CO2 <-  nls(O2rate_chl ~ O2max*CO2adj/(Km+CO2adj), Pt4[which(Pt4$inhibitor == "none"),], start=list(O2max=top,Km=kco2))
Pt4_nls_CO2_AZ <-  nls(O2rate_chl ~ O2max*CO2adj/(Km+CO2adj), Pt4[which(Pt4$inhibitor == "AZ"),], start=list(O2max=top,Km=kco2))
Pt4_nls_CO2_EZ <-  nls(O2rate_chl ~ O2max*CO2adj/(Km+CO2adj), Pt4[which(Pt4$inhibitor == "EZ"),], start=list(O2max=top,Km=kco2))
#list the models
...
residual_data_20 <- data.frame(
  Fitted = c(fitted(Pt20_nls_CO2), fitted(Pt20_nls_CO2_AZ), fitted(Pt20_nls_CO2_EZ)),
  Residuals = c(residuals(Pt20_nls_CO2), residuals(Pt20_nls_CO2_AZ), residuals(Pt20_nls_CO2_EZ)),
  Inhibitor = rep(c("None", "AZ", "EZ"), 
              times = c(length(residuals(Pt20_nls_CO2)),
                        length(residuals(Pt20_nls_CO2_AZ)), 
                        length(residuals(Pt20_nls_CO2_EZ)))))
residual_data_4 <- data.frame(
  Fitted = c(fitted(Pt4_nls_CO2), fitted(Pt4_nls_CO2_AZ), fitted(Pt4_nls_CO2_EZ)),
  Residuals = c(residuals(Pt4_nls_CO2), residuals(Pt4_nls_CO2_AZ), residuals(Pt4_nls_CO2_EZ)),
  Inhibitor = rep(c("None", "AZ", "EZ"), 
              times = c(length(residuals(Pt4_nls_CO2)),
                        length(residuals(Pt4_nls_CO2_AZ)), 
                        length(residuals(Pt4_nls_CO2_EZ)))))

# Reorder the factor levels and plot using pipes
residual20 <- ggplot(residual_data_20 %>% mutate(Inhibitor = factor(Inhibitor, levels = c("None", "AZ", "EZ"))),
         aes(x = Fitted, y = Residuals)) +
  geom_point(colour = "black") +
  geom_hline(yintercept = 0, linetype = "dashed", color = "red") +
  facet_wrap(~ Inhibitor, nrow = 1) +
  labs(x = "Fitted Values", y = "Residuals") +
  theme_bw()

residual4 <- ggplot(residual_data_4 %>% mutate(Inhibitor = factor(Inhibitor, levels = c("None", "AZ", "EZ"))),
                     aes(x = Fitted, y = Residuals)) +
  geom_point(colour = "black") +
  geom_hline(yintercept = 0, linetype = "dashed", color = "red") +
  facet_wrap(~ Inhibitor, nrow = 1) +
...
# plotting up in ggplot
CO2_chl_plot_Pt4 <- ggplot(Pt4, aes(x = CO2adj, y = O2rate_chl)) +
  theme_bw() +
  geom_ribbon(aes(ymin = lw4_CO2, ymax = up4_CO2), fill = "#D51317", colour= NA, alpha = 0.2) +
  geom_ribbon(aes(ymin = lwAZ4_CO2, ymax = upAZ4_CO2), fill = "#95C11F", colour = NA, alpha = 0.2) +
  geom_ribbon(aes(ymin = lwEZ4_CO2, ymax = upEZ4_CO2), fill = "#0094CD", colour= NA, alpha = 0.2) +
  geom_point(aes(fill = inhibitor, shape = inhibitor), size = 2,colour = "black", ) +
  scale_fill_manual(values = c("#D51317FF" ,"#95C11FFF","#0094CDFF")) +
  scale_shape_manual(values = c(23,23,23)) +
  ylab(bquote('\u03bcmol'~O[2]~hr^-1~mg~Chl~italic(a)^-1)) + 
  xlab(bquote('CO2 (\u03bcmol'~L^-1~')')) +
  scale_y_continuous(limits = c(-60, 180), breaks = seq(-60, 180, by = 30)) +
...
DIC_chl_plot_Pt4 <- ggplot(Pt4, aes(x = DICadj, y = O2rate_chl)) +
  theme_bw() +
  geom_ribbon(aes(ymin = lw4_DIC, ymax = up4_DIC), fill = "#D51317", colour= NA, alpha = 0.2) +
  geom_ribbon(aes(ymin = lwAZ4_DIC, ymax = upAZ4_DIC), fill = "#95C11F", colour = NA, alpha = 0.2) +
  geom_ribbon(aes(ymin = lwEZ4_DIC, ymax = upEZ4_DIC), fill = "#0094CD", colour= NA, alpha = 0.2) +
  geom_point(aes(fill = inhibitor, shape = inhibitor), size = 2,colour = "black", ) +
  scale_fill_manual(values = c("#D51317FF" ,"#95C11FFF","#0094CDFF")) +
  scale_shape_manual(values = c(23,23,23)) +
  ylab(bquote('\u03bcmol'~O[2]~hr^-1~mg~Chl~italic(a)^-1)) + 
...
Boot_plusdata_Pt4 <- ggplot() +
  ...
  theme(axis.text = element_text(size = 10, colour = "black"),
        ...
        legend.text = element_text(size = 9), 
        legend.direction = "horizontal", legend.position = "none") +
  scale_x_continuous(limits = c(0, 2500), breaks = seq(0, 2500, by = 500)) +
  scale_y_continuous(limits = c(80, 200), breaks = seq(80, 200, by = 20)) +
  ...
  geom_errorbarh(data = avgcoef_Pt4 %>% filter(CoefficientType == "DIC"), aes(y = VmaxO2, xmin = K - Kse, xmax = K + Kse, colour = inhibitor), colour = "black", height = 3, size = 0.4) +
...
#fitting MM kinetics to whole data set
Pi_nls_DIC <-  nls(O2rate_chl ~ O2max*DICadj/(Km+DICadj), Pi[which(Pi$inhibitor == "none"),], start=list(O2max=top,Km=kdic))
Pi_nls_DIC_AZ <-  nls(O2rate_chl ~ O2max*DICadj/(Km+DICadj), Pi[which(Pi$inhibitor == "AZ"),], start=list(O2max=top,Km=kdic))
Pi_nls_DIC_EZ <-  nls(O2rate_chl ~ O2max*DICadj/(Km+DICadj), Pi[which(Pi$inhibitor == "EZ"),], start=list(O2max=top,Km=kdic))
Pi_nls_CO2 <-  nls(O2rate_chl ~ O2max*CO2adj/(Km+CO2adj), Pi[which(Pi$inhibitor == "none"),], start=list(O2max=top,Km=kco2))
Pi_nls_CO2_AZ <-  nls(O2rate_chl ~ O2max*CO2adj/(Km+CO2adj), Pi[which(Pi$inhibitor == "AZ"),], start=list(O2max=top,Km=kco2))
Pi_nls_CO2_EZ <-  nls(O2rate_chl ~ O2max*CO2adj/(Km+CO2adj), Pi[which(Pi$inhibitor == "EZ"),], start=list(O2max=top,Km=kco2))

Pi_nls_DIC_DIDS <-  nls(O2rate_chl ~ O2max*DICadj/(Km+DICadj), Pi[which(Pi$inhibitor == "DIDS"),], start=list(O2max=top,Km=kdic))
Pi_nls_CO2_DIDS <-  nls(O2rate_chl ~ O2max*CO2adj/(Km+CO2adj), Pi[which(Pi$inhibitor == "DIDS"),], start=list(O2max=top,Km=kco2))
...
models_Pi <- list(Pi_nls_DIC = Pi_nls_DIC,
                   ...
                   Pi_nls_DIC_DIDS = Pi_nls_DIC_DIDS,
                   Pi_nls_CO2_DIDS = Pi_nls_CO2_DIDS)
...
MMDIDSPi_DIC <- function(x) MM_fit_Pi(x, "DIC", "DIDS")
MMDIDSPi_CO2 <- function(x) MM_fit_Pi(x, "CO2", "DIDS")
#functions for confidence intervals
# Helper function to get coefficients and confidence intervals
...
upDIDSPi_DIC <- sapply(Pi$DICadj, FUN = function(x) get_coef_conf_Pi("DIC", "DIDS", "O2max", "97.5%") * x / (get_coef_conf_Pi("DIC", "DIDS", "Km", "2.5%") + x))
lwDIDSPi_DIC <- sapply(Pi$DICadj, FUN = function(x) get_coef_conf_Pi("DIC", "DIDS", "O2max", "2.5%") * x / (get_coef_conf_Pi("DIC", "DIDS", "Km", "97.5%") + x))
...
upDIDSPi_CO2 <- sapply(Pi$CO2adj, FUN = function(x) get_coef_conf_Pi("CO2", "DIDS", "O2max", "97.5%") * x / (get_coef_conf_Pi("CO2", "DIDS", "Km", "2.5%") + x))
lwDIDSPi_CO2 <- sapply(Pi$CO2adj, FUN = function(x) get_coef_conf_Pi("CO2", "DIDS", "O2max", "2.5%") * x / (get_coef_conf_Pi("CO2", "DIDS", "Km", "97.5%") + x))


# plotting up in ggplot
CO2_chl_plot_Pi <- ggplot(Pi, aes(x = CO2adj, y = O2rate_chl)) +
  theme_bw() +
  geom_ribbon(aes(ymin = lwPi_CO2, ymax = upPi_CO2), fill = "#D51317", colour= NA, alpha = 0.2) +
  geom_ribbon(aes(ymin = lwAZPi_CO2, ymax = upAZPi_CO2), fill = "#95C11F", colour = NA, alpha = 0.2) +
  geom_ribbon(aes(ymin = lwEZPi_CO2, ymax = upEZPi_CO2), fill = "#0094CD", colour= NA, alpha = 0.2) +
  geom_point(aes(fill = inhibitor, shape = inhibitor), size = 2,colour = "black", ) +
  scale_fill_manual(values = c("#D51317FF" ,"#95C11FFF","#0094CDFF")) +
  scale_shape_manual(values = c(23,23,23)) +
  ylab(bquote('\u03bcmol'~O[2]~hr^-1~mg~Chl~italic(a)^-1)) + 
  xlab(bquote('CO2 (\u03bcmol'~L^-1~')')) +
  scale_y_continuous(limits = c(-50, 150), breaks = seq(-100, 150, by = 25)) +
  scale_x_continuous(limits = c(0, 29.513), breaks = seq(0, 30, by = 5)) +
  theme(axis.text = element_text(size = 10, colour = "black"),
        panel.grid.minor = element_blank(), 
        legend.text = element_text(size = 9), 
...
DIC_chl_plot_Pi <- ggplot(Pi, aes(x = DICadj, y = O2rate_chl)) +
  theme_bw() +
  geom_ribbon(aes(ymin = lwPi_DIC, ymax = upPi_DIC), fill = "#D51317", colour= NA, alpha = 0.2) +
  geom_ribbon(aes(ymin = lwAZPi_DIC, ymax = upAZPi_DIC), fill = "#95C11F", colour = NA, alpha = 0.2) +
  geom_ribbon(aes(ymin = lwEZPi_DIC, ymax = upEZPi_DIC), fill = "#0094CD", colour= NA, alpha = 0.2) +
  geom_point(aes(fill = inhibitor, shape = inhibitor), size = 2,colour = "black", ) +
  scale_fill_manual(values = c("#D51317FF" ,"#95C11FFF","#0094CDFF")) +
  scale_shape_manual(values = c(23,23,23)) +
  ylab(bquote('\u03bcmol'~O[2]~hr^-1~mg~Chl~italic(a)^-1)) + 
...
# plotting up in ggplot
DIC_chl_plot_Pi_DIDS <- ggplot(Pi, aes(x = DICadj, y = O2rate_chl)) +
  theme_bw() +
  geom_ribbon(aes(ymin = lwPi_DIC, ymax = upPi_DIC), fill = "#D51317", colour= NA, alpha = 0.2) +
  geom_ribbon(aes(ymin = lwDIDSPi_DIC, ymax = upDIDSPi_DIC), fill = "#C27DFF", colour = NA, alpha = 0.2) +
  geom_point(aes(fill = inhibitor, shape = inhibitor), size = 2,colour = "black", ) +
  scale_fill_manual(values = c("#D51317FF","#95C11FFF","#0094CDFF","#C27DFFFF")) +
  scale_shape_manual(values = c(23,23,23,23)) +
  ylab(bquote('\u03bcmol'~O[2]~hr^-1~mg~Chl~italic(a)^-1)) + 
  xlab(bquote('DIC (\u03bcmol'~L^-1~')')) +
  scale_y_continuous(limits = c(-50, 175), breaks = seq(-100, 175, by = 25)) +
  scale_x_continuous(limits = c(0, 4000), breaks = seq(0, 4000, by = 500)) +
  theme(axis.text = element_text(size = 10, colour = "black"),
        panel.grid.minor = element_blank(), 
        legend.text = element_text(size = 9), 
  ...
  # plot the lines    
  stat_function(fun = MMPi_DIC, colour = "#D51317FF", size = 0.7) +
  stat_function(fun = MMDIDSPi_DIC, colour = "#C27DFFFF", size = 0.7) 

ggsave("Pi_MM_DIDSplot2.svg", DIC_chl_plot_Pi_DIDS, width = 3.8, height = 3.5)
...
boot_Pi_DIC <- list(
  ...
  boots_DIC_DIDS = data.frame(x = boot_models_Pi$Pi_nls_DIC_DIDS$coefboot[, 1], y = boot_models_Pi$Pi_nls_DIC_DIDS$coefboot[, 2])
)
...
Boot_plusdata_Pi <- ggplot() +
  ...
  geom_errorbarh(data = avgcoef_Pi %>% filter(CoefficientType == "DIC"), aes(y = VmaxO2, xmin = K - Kse, xmax = K + Kse, colour = inhibitor), colour = "black", height = 3, size = 0.4) +
  ...
  geom_point(data = avgcoef_Pi %>% filter(CoefficientType == "DIC"), aes(y = VmaxO2, x = K, fill = inhibitor), colour = "black", shape = 22, size = 4) +
  scale_fill_manual(values = c("none" = "#D51317FF", "AZ" = "#95C11FFF", "EZ" = "#0094CDFF")) + 
  scale_colour_manual(values = c("none" = "#D5131755", "AZ" = "#95C11F55", "EZ" = "#0094CD55")) +
  xlab(bquote(K[0.5~~DIC]~'(\u03bcM)')) + 
  ylab(bquote(VO[2]^'max'~'(\u03bcmol'~O[2]~hr^-1~mg~Chl~italic(a)^-1~')')) 

Boot_plusdata_Pi_DIDS <- ggplot() +
  theme_bw() +
  theme(axis.text = element_text(size = 10, colour = "black"),
        panel.grid.minor = element_blank(), 
        legend.text = element_text(size = 9), 
  ...
  scale_y_continuous(limits = c(80, 200), breaks = seq(80, 200, by = 20)) +
  geom_point(data = boot_Pi_DIC$boots_DIC, aes(y = x, x = y), color = "#D51317FF", shape = 3) +
  geom_point(data = boot_Pi_DIC$boots_DIC_DIDS, aes(y = x, x = y), color = "#C27DFFFF", shape = 3) +
  geom_point(data = assay_table_Pi, aes(y = estimate_O2max, x = estimate_Km, fill = inhibitor), colour = "black", shape = 23, size = 2) +
  geom_errorbarh(data = avgcoef_Pi %>% filter(CoefficientType == "DIC") %>% filter(inhibitor %in% c("none", "DIDS")), aes(y = VmaxO2, xmin = K - Kse, xmax = K + Kse, colour = inhibitor), colour = "black", height = 3, size = 0.4) +
  geom_errorbar(data = avgcoef_Pi %>% filter(CoefficientType == "DIC")%>% filter(inhibitor %in% c("none", "DIDS")), aes(x = K, ymin = VmaxO2 - O2se, ymax = VmaxO2 + O2se, colour = inhibitor), colour = "black", width = 40, size = 0.4) +
  geom_point(data = avgcoef_Pi %>% filter(CoefficientType == "DIC") %>% filter(inhibitor %in% c("none", "DIDS")), aes(y = VmaxO2, x = K, fill = inhibitor), colour = "black", shape = 22, size = 4) +
  scale_fill_manual(values = c("none" = "#D51317FF", "DIDS" = "#C27DFFFF")) + 
  scale_colour_manual(values = c("none" = "#D5131755", "DIDS" = "#C27DFF55")) +
  xlab(bquote(K[0.5~~DIC]~'(\u03bcM)')) + 
  ylab(bquote(VO[2]^'max'~'(\u03bcmol'~O[2]~hr^-1~mg~Chl~italic(a)^-1~')')) 

ggsave("bootstraps_DIDS.svg", Boot_plusdata_Pi_DIDS, width = 3.8, height = 3.5)
...
#fitting MM kinetics to whole data set
Cf_nls_DIC <-  nls(O2rate_chl ~ O2max*DICadj/(Km+DICadj), Cf[which(Cf$inhibitor == "none"),], start=list(O2max=top,Km=kdic))
Cf_nls_DIC_AZ <-  nls(O2rate_chl ~ O2max*DICadj/(Km+DICadj), Cf[which(Cf$inhibitor == "AZ"),], start=list(O2max=top,Km=kdic))
Cf_nls_DIC_EZ <-  nls(O2rate_chl ~ O2max*DICadj/(Km+DICadj), Cf[which(Cf$inhibitor == "EZ"),], start=list(O2max=top,Km=kdic))
Cf_nls_CO2 <-  nls(O2rate_chl ~ O2max*CO2adj/(Km+CO2adj), Cf[which(Cf$inhibitor == "none"),], start=list(O2max=top,Km=kco2))
Cf_nls_CO2_AZ <-  nls(O2rate_chl ~ O2max*CO2adj/(Km+CO2adj), Cf[which(Cf$inhibitor == "AZ"),], start=list(O2max=top,Km=kco2))
Cf_nls_CO2_EZ <-  nls(O2rate_chl ~ O2max*CO2adj/(Km+CO2adj), Cf[which(Cf$inhibitor == "EZ"),], start=list(O2max=top,Km=kco2))
#list the models
...
# plotting up in ggplot
CO2_chl_plot_Cf <- ggplot(Cf, aes(x = CO2adj, y = O2rate_chl)) +
  theme_bw() +
  geom_ribbon(aes(ymin = lwCf_DIC, ymax = upCf_DIC), fill = "#D51317", colour= NA, alpha = 0.2) +
  geom_ribbon(aes(ymin = lwAZCf_DIC, ymax = upAZCf_DIC), fill = "#95C11F", colour = NA, alpha = 0.2) +
  geom_ribbon(aes(ymin = lwEZCf_DIC, ymax = upEZCf_DIC), fill = "#0094CD", colour= NA, alpha = 0.2) +
  geom_point(aes(fill = inhibitor, shape = inhibitor), size = 2,colour = "black", ) +
  scale_fill_manual(values = c("#D51317FF" ,"#95C11FFF","#0094CDFF")) +
  scale_shape_manual(values = c(23,23,23)) +
  ylab(bquote('\u03bcmol'~O[2]~hr^-1~mg~Chl~italic(a)^-1)) + 
  xlab(bquote('CO2 (\u03bcmol'~L^-1~')')) +
  scale_y_continuous(limits = c(-30, 90), breaks = seq(-30, 90, by = 15)) +
  scale_x_continuous(limits = c(0, 29.513), breaks = seq(0, 30, by = 5)) +
  theme(axis.text = element_text(size = 10, colour = "black"),
...
DIC_chl_plot_Cf <- ggplot(Cf, aes(x = DICadj, y = O2rate_chl)) +
  theme_bw() +
  geom_ribbon(aes(ymin = lwCf_DIC, ymax = upCf_DIC), fill = "#D51317", colour= NA, alpha = 0.2) +
  geom_ribbon(aes(ymin = lwAZCf_DIC, ymax = upAZCf_DIC), fill = "#95C11F", colour = NA, alpha = 0.2) +
  geom_ribbon(aes(ymin = lwEZCf_DIC, ymax = upEZCf_DIC), fill = "#0094CD", colour= NA, alpha = 0.2) +
...
Bootplot_Cf <- ggplot() +
  ...
  scale_x_continuous(limits = c(50, 175), breaks = seq(50, 175, by = 12.5)) +
...
Boot_plusdata_Cf <- ggplot() +
  ...
  scale_y_continuous(limits = c(50, 170), breaks = seq(50, 170, by = 10)) +
  ...
  geom_errorbarh(data = avgcoef_Cf %>% filter(CoefficientType == "DIC"), aes(y = VmaxO2, xmin = K - Kse, xmax = K + Kse, colour = inhibitor), colour = "black", height = 3, size = 0.4) +
  geom_errorbar(data = avgcoef_Cf %>% filter(CoefficientType == "DIC"), aes(x = K, ymin = VmaxO2 - O2se, ymax = VmaxO2 + O2se, colour = inhibitor), colour = "black", width = 200, size = 0.4) +
  geom_point(data = avgcoef_Cf %>% filter(CoefficientType == "DIC"), aes(y = VmaxO2, x = K, fill = inhibitor), colour = "black", shape = 22, size = 4) +
  scale_fill_manual(values = c("none" = "#D51317FF", "AZ" = "#95C11FFF", "EZ" = "#0094CDFF")) + 
  scale_colour_manual(values = c("none" = "#D5131755", "AZ" = "#95C11F55", "EZ" = "#0094CD55")) +
  xlab(bquote(K[0.5~~DIC])) + 
  ylab(bquote(VO[2]^'max'~'(\u03bcmol'~O[2]~hr^-1~mg~Chl~italic(a)^-1~')')) 
...
ACi_DIC_chl <- guide_area() / (DIC_chl_plot_Pt20 | DIC_chl_plot_Pt4) / (DIC_chl_plot_Pi_DIDS | DIC_chl_plot_Cf) +
...
ACi_CO2_chl <- guide_area() / (CO2_chl_plot_Pt20 | CO2_chl_plot_Pt4) / (CO2_chl_plot_Pi | CO2_chl_plot_Cf) +
  plot_layout(guides = 'collect', nrow = (4), heights = c(1,10,10)) +
  plot_annotation(tag_levels = list(c('a','b','c', 'd'))) &
  theme(axis.text = element_text(size = 10, colour = "black"), 
        legend.title = element_text(size = 9), legend.text = element_text(size = 9), 
...
ggsave("ACi_chl_DIC_plot_3.svg", ACi_DIC_chl, width = 7.2, height = 7.4)
ggsave("ACi_chl_CO2_plot_2.svg", ACi_CO2_chl, width = 7.2, height = 7.4)

MM_fit_boot_plots <- guide_area() / (Boot_plusdata_Pt20 | Boot_plusdata_Pt4) / (Boot_plusdata_Pi | Boot_plusdata_Cf) +
  plot_layout(guides = 'collect', nrow = (4), heights = c(1,10,10)) +
  plot_annotation(tag_levels = list(c('a','b','c', 'd'))) &
  theme(axis.text = element_text(size = 10, colour = "black"), 
        legend.title = element_text(size = 9), legend.text = element_text(size = 9), 
...
ggsave("MM_fit_boot_plot_2.svg", MM_fit_boot_plots, width = 7.2, height = 7.4)
